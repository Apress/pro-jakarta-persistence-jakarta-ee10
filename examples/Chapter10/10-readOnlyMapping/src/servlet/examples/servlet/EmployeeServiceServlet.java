...
public class EmployeeServiceServlet extends HttpServlet {
    ...
    private final String TITLE = 
        "Chapter 10: Read-only Mapping Example";
    
    private final String DESCRIPTION = 
         "This example demonstrates a way to map an entity as read-only. Changing any of the employees " +
         "will have no effect on their state in the database.";
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        } else if (action.equals("ModifyEmp")) {
            Employee emp = empService.modifyEmployee(
                    parseInt(request.getParameter("empId")),
                    request.getParameter("empName"),
                    parseLong(request.getParameter("empSal")));
            out.println("Tried to modify employee " + emp + ". ");
            out.println("Use findAll query to see if employee was modified.");
    ...
    private void printHtmlHeader(PrintWriter out) throws IOException {
        ...
        out.println("<h3>Modify an Employee</h3>");
        ...
        out.println("<tr><td>Employee Salary:</td><td><input type=\"text\" name=\"empSal\"/>(long)</td></tr>");
        out.println("<tr><td><input name=\"action\" type=\"submit\" value=\"ModifyEmp\"/></td></tr>");
